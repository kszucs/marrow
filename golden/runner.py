...
_DEF_RE = re.compile(r"^def\s+(" + PREFIX + r"\w+)\(\)\s+raises:$")


class Case:
    """One `golden/cases/<name>.mojo`.

    `sql` is the docstring's first paragraph, `prose` whatever follows it, and
    `expected` the typed table after the `-- expected` marker. `body` is the
    Mojo source below the docstring, dedented — the one spelling both lanes
    run.
    """

    def __init__(self, path, name, sql, prose, expected, body, skips):
        self.path = path
        self.name = name
        self.sql = sql
        self.prose = prose
...
def parse_case(path):
    ...
    if not lines:
        raise SystemExit(f"golden: {path.name}: empty case file")

    match = _DEF_RE.match(lines[0])
    if match is None:
        raise SystemExit(
            f"golden: {path.name}: first line must be "
            f"`def {PREFIX}<name>() raises:`, got {lines[0]!r}"
        )
    name = match.group(1)
    if name != PREFIX + path.stem:
        raise SystemExit(
            f"golden: {path.name}: defines {name!r}, which does not match "
            f"the file name; expected {PREFIX + path.stem!r}"
        )
    ...
    if len(lines) < 2 or lines[1].strip() != '"""':
        ...
        raise SystemExit(
            f'golden: {path.name}: line 2 must be a lone `"""` opening the docstring'
        )
    for end, line in enumerate(lines[2:], start=2):
        if line.strip() == '"""':
            break
    else:
        raise SystemExit(f"golden: {path.name}: docstring is never closed")

    doc = [_dedent(line) for line in lines[2:end]]
    ...
    return Case(path, name, sql, prose, expected, body, skips)
...
MOJO_HEADER = '''"""The golden corpus, one function per case.

GENERATED by `golden/runner.py` from `golden/cases/*.mojo` — do not edit.
Regenerate expectations with `pixi run -e bench python golden/runner.py`.
"""

from golden.helpers import check, table
...
def _name_check(body, name):
    """`check(q)` -> `check("<case>", q)`.

    Mojo cannot introspect its own function name, so both lanes inject it the
    same way. The case text stays lane-neutral, and the rewrite is textual
    precisely because `check(q)` is a short single line.
    """
    injected, count = re.subn(r"\bcheck\(", f'check("{name}", ', body)
    if count != 1:
        raise SystemExit(
            f"golden: {name}: body must call `check(...)` exactly once, found {count}"
        )
    return injected


def generate_mojo(cases):
    """Concatenate the cases into one collectable `test_*.mojo` module.

    One module rather than one per case: the harness generates a single driver
    for the whole selection and compilation is dominated by elaborating marrow,
    so module count is the only part of the cost this controls.
    """
    blocks = [MOJO_HEADER]
    for case in cases:
        ...
        blocks.append(
            ...
            f"{_indent(doc)}\n"
            f'    """\n'
            f"{_indent(_name_check(case.body, case.name))}\n"
        )
    text = "\n\n".join(blocks)
...
def transpile(case):
    """The case body as Python source.

    Three rules — drop `raises`, drop `var`, keep everything else — which hold
    only while case bodies stay inside the intersection of the two grammars.
    That costs two constraints, both checked by review rather than here: no
    type annotations (`var t: DynRelation = ...` would need `DynRelation` in
    the Python namespace) and no transfer sigils (`q^` has no Python reading,
    so every helper a case calls takes its arguments borrowed).
    """
    body = _name_check(case.body, case.name)
    lines = [_VAR_RE.sub(r"\1", line) for line in body.split("\n")]
    doc = case.sql if not case.prose else f"{case.sql}\n\n{case.prose}"
    return (
        f"def {case.name}():\n"
        f'    """\n'
...
def install(namespace, target, cases=None):
    """Compile every case against `namespace` and inject it into `target`.
    ...
    for case in load_cases() if cases is None else cases:
        source = transpile(case)
        code = compile(source, str(case.path), "exec")
        scope = dict(namespace)
        exec(code, scope)
        function = scope[case.name]
