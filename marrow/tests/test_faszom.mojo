...
def test_boolean_operators() raises:
    ...
    var r_not = t.filter(~(t.a > lit(int32, 5))).execute(batch)
    assert_true(
        r_not.column("data") == arange[Int32Type](0, 6)
    )  # [0, 1, 2, 3, 4, 5]
...
def test_schema_filter_reuse() raises:
    ...
    assert_true(
        rel.execute(batch1).column("data") == arange[Int32Type](n // 2, n)
    )
    assert_true(
        rel.execute(batch2).column("data") == arange[Int32Type](n, 2 * n)
    )
...
def test_field_convenience_same_as_field_bracket() raises:
    """field["a"](int32) produces the same Field as Field["a"](int32)."""
    var f1 = field["a"](int32)
    var f2 = Field["a"](int32)
    assert_equal(f1._name_matches["a"](), f2._name_matches["a"]())


def test_table_convenience() raises:
    """table(field("a", int32), field("b", int32)) works as Schema constructor.
    """
    var n = 10
    var batch = _make_batch(n)
    ...
    var r1 = t1.filter(t1.a + t1.b > t1.c).execute(batch)
    ...
    var r2 = t2.filter(t2.a + t2.b > t2.c).execute(batch)

    assert_true(r1.column("data") == r2.column("data"))
    assert_true(r1.column("a") == r2.column("a"))
