"""Shared dispatch helpers for compute kernels.
...
Provides:
  ...
  - `bool_array_dispatch` — runtime-typed dispatch producing BoolArray output.
  - `unary_numeric_dispatch` — runtime-typed unary dispatch over numeric dtypes.
  - `binary_float_dispatch` — runtime-typed binary dispatch restricted to floats.
...
def unary_numeric_dispatch[
    ...
    Args:
        ...
        ctx: Execution context; forwarded to the typed kernel.

    Returns:
        A new AnyArray with the element-wise result.
    """
    if array.dtype() == int8:
        return func(array.as_int8(), ctx).to_any()
    elif array.dtype() == int16:
        return func(array.as_int16(), ctx).to_any()
    elif array.dtype() == int32:
        return func(array.as_int32(), ctx).to_any()
    elif array.dtype() == int64:
        return func(array.as_int64(), ctx).to_any()
    elif array.dtype() == uint8:
        return func(array.as_uint8(), ctx).to_any()
    elif array.dtype() == uint16:
        return func(array.as_uint16(), ctx).to_any()
    elif array.dtype() == uint32:
        return func(array.as_uint32(), ctx).to_any()
    elif array.dtype() == uint64:
        return func(array.as_uint64(), ctx).to_any()
    elif array.dtype() == float16:
        return func(array.as_float16(), ctx).to_any()
    elif array.dtype() == float32:
        return func(array.as_float32(), ctx).to_any()
    elif array.dtype() == float64:
        return func(array.as_float64(), ctx).to_any()
