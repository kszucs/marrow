...
- ``DecimalRescaleKernel`` / ``IntToDecimalKernel`` / ``DecimalToIntKernel`` /
  ``FloatToDecimalKernel`` / ``DecimalToFloatKernel``, behind the
  ``DecimalCastKernel`` router — a decimal is an integer plus a scale, so each
  is a rescale by a power of ten. One struct per *conversion*, not per dtype
  pair; that section's banner comment carries the measurement that forced it.
...
struct NumericCastKernel(CastKernel):
    ...
    def apply[
        ...
        return PrimitiveArray[To](
            ...
            nulls=array.nulls,
            offset=0,
            bitmap=_rebased_validity(array.bitmap, array.offset, length),
...
def _rebased_validity(
    bitmap: Optional[Bitmap[mut=False]], offset: Int, length: Int
) raises -> Optional[Bitmap[mut=False]]:
    """The source's validity bitmap, indexed from 0 instead of from `offset`.

    Every cast that allocates a fresh values buffer starts it at 0 while
    sharing the source's bitmap, which is indexed from the source's own
    offset. For a sliced input the two disagree and every null lands on the
    wrong row. An offset-0 source shares as before; anything else pays one
    copy, which is what `ArrayData.owned_validity` documents itself as being
    for.
    """
    if offset == 0 or not bitmap:
        return bitmap.copy()
...
def _digit_bound[T: DType](dt: DynType) raises -> Scalar[T]:
    ...
    limits. Without it `cast(x, decimal128(5, 0), safe=True)` returns a
...
def _map_decimal[
    ...
    *valid* element converts cleanly. ``NumericCastKernel.apply`` masks its checked
    ...
    ``validity`` is built inline rather than through ``ArrayData.validity()``
    -- which is the same view, minus the ``nulls == 0`` short-circuit --
    because calling the method cost +89,600 bytes of ``__text`` on
    ``query_dynvalue`` and +89,472 on ``query_streaming_agg``. Its origin comes
    through a ``ref self`` projection, so the cost lands in this function's
    ``[FromN, ToN]`` matrix. Same for ``_scale_checked``.
    """
    var n = data.length
    ...
    return DynArray.from_data(
        ArrayData(
            ...
            nulls=data.nulls,
            offset=0,
            bitmap=_rebased_validity(data.bitmap, data.offset, n),
...
def _rescale_up[
    ...
    """
    var f = pow(Scalar[ToN](10), delta)
    # Hoisted: loop-invariant, and two fewer divisions per element.
    var hi = Scalar[ToN].MAX // f
    # `//` floors, so the *lower* bound needs ceil(MIN / f); the floored value
    # is one step too far out and lets a value through that wraps on multiply.
...
def _rescale_down[
    ...
    def down(x: Scalar[FromN]) raises {imm} -> Scalar[ToN]:
        var r = x % f
        var q = x // f
        # `//` floors; Arrow's ReduceScaleBy(round=false) truncates toward
        # zero, so a negative value with a remainder is one step too low.
        if r != 0 and x < 0:
            q += 1
        if safe:
            if r != 0:
...
struct TemporalCastKernel(CastKernel):
    ...
    def ns_per_tick(dt: DynType) raises -> Int64:
        ...
        **reverted**: it removed 49 source lines and cost **+10,560 bytes of
        ...
        shape as the `DecimalCastKernel._convert` merge recorded above. This change,
    ...
    def _scale_checked[
        ...
        valid array. The inline validity view is measured -- see
        `_map_decimal`."""
        var n = data.length
        var validity = Optional[BitmapView[origin_of(data.bitmap._value)]](None)
        if data.bitmap and data.nulls > 0:
