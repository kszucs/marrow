...
It lives here with its producer rather than in a module of its own, so the
aggregate path imports one name from one place.
...
struct Groups(Copyable, Movable):
    ...
    the strategy that produced it — `HashGrouping` below is one such strategy,
    and `expr` picks between hashing and the implicit one slot at plan time.
    """
