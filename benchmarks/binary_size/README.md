...
That conclusion is still the architecture today (`marrow.expr.logical`'s
immutable plan nodes lowering to `marrow.expr.physical` operators, and
`DynValue`'s fused-or-interpreted box) — see
