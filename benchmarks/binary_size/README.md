...
always included as the ratio baseline). `devkit/footprint.py` holds the gate
list, the module buckets and the measurement (it needs `mojo`, `nm`, `size` and
`strip` on `$PATH`) — read it directly if you want to change what gets
measured.

`pixi run binary_size_check` is the CI gate: it rebuilds the subset
...
`devkit/footprint.py` now reports and ratios on `__text`. Two consequences for
what follows:
...
`binary_size_check --update`, so it cannot go stale as prose the way this table
