...
Or install DataFusion into an existing environment (`pip install datafusion`)
and import:
...
Any Marrow compute function can be wrapped as a DataFusion UDF. The two
conversions — `ma.array(pa_arr)` in and `pa.array(ma_arr)` out — hand the
buffers over through the Arrow C Data Interface instead of copying the values.
Each call still pays the usual Python call overhead.

```{python}
def mojo_add(a: pa.Array, b: pa.Array) -> pa.Array:
    return pa.array(mc.add(ma.array(a), ma.array(b)))
...
2. `ma.array(a)` and `ma.array(b)` import each argument into Marrow via `__arrow_c_array__` — a pointer exchange, no copy — and `mc.add` runs the vectorized Mojo kernel over the Arrow buffers.
3. `pa.array(result)` exports the result back to PyArrow via `__arrow_c_array__` — again, no copy.

The data layout is identical in both libraries, so the import and export are O(1) pointer operations regardless of array length.
