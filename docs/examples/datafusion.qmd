...
conversions — `ma.array(pa_arr)` in and `pa.array(ma_arr)` out — are zero-copy
pointer exchanges over the Arrow C Data Interface, not serialization, so the
wrapper costs nothing beyond the kernel itself.
...
2. `mc.add(a, b)` imports each argument into Marrow via `__arrow_c_array__` — a pointer exchange, no copy — and runs the SIMD-vectorised Mojo kernel over the raw Arrow buffer.
