...
`how=` takes `inner`, `left`, `right`, `full`, `semi` and `anti`. Use
...
- **`read_parquet` reads one file** (local, or an object-store URI through
  OpenDAL): no globs, directories or hive partitions, and no CSV or JSON.
- **Row groups are skipped only after `.optimize()`.** Without it, a filtered
  scan reads every row group.
- **No `distinct`/`union`/`except`/`intersect` nodes**, and no regex.
- **Comparisons follow SQL, not IEEE**: `nan = nan` is true here and false in
  `marrow.compute` — see [Compute](compute.qmd).
