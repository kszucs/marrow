...
print("is_valid:  ", arr.is_valid())     # elementwise BooleanArray
print("is_valid(0):", arr[0].is_valid())   # True
print("is_valid(1):", arr[1].is_valid())   # False — null
```

::: {.callout-note}
**Null is not zero.** `is_valid()` reports `False` at index 1 — the value there is absent entirely, not 0 or empty. `Array.is_valid()` returns an elementwise `BooleanArray` (like `pyarrow.compute.is_valid`); index into the array first (`arr[1]`) to check a single position.
:::
