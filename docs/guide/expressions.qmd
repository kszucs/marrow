...
```python
projected = adults.select(col("name"), col("age"), col("age") - lit(18))
```

Note: there is no `alias()` yet — output column names follow the structure of the expression tree.
...
|-----------|---------|-------------|
| `morsel_size` | 65536 | Number of rows per output batch |
