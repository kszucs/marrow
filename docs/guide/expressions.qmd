...
`execute(plan)` opens the plan into a fresh pull-based processor tree, drains
it, and returns a single `RecordBatch`. A plan is a pure description, so it can
be executed repeatedly. An optional `ExecContext` selects the execution
mode:

```python
from marrow.expr import execute, ExecContext

result = execute(plan)                       # serial by default
result = execute(plan, ExecContext())   # explicit context
```
...
```python
from marrow.expr import col, lit, parquet_scan, execute
