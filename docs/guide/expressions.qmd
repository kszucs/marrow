...
be executed repeatedly. An optional `ExecutionContext` selects the execution
mode:
...
result = execute(plan, ExecutionContext())   # explicit context
```

`ExecutionContext` comes from `marrow.kernels.execution` and offers
