...
| **B23** | **Harness half RESOLVED; the toolchain hang itself remains.** `run_with_progress` called `proc.communicate()` with no timeout, so a hung Mojo process blocked the run forever — `MojoRunner.collect` splits and retries on a compiler *crash*, which emits a signal, but a hang emits none. Added `--mojo-timeout SECONDS` (default 1800, 0 disables): the process is killed, the failure carries a message telling the reader to compare `etime` against `time` to confirm a hang rather than slowness, and the returncode is forced to 124 so callers checking `!= 0` treat it like any other failure. Verified 2026-08-05: `--mojo-timeout=90` on `test_join.mojo` alone terminates in 90s and reports 28 failures, where it previously ran 7 hours on 10.4s of CPU. **Still open:** why that single-file unit deadlocks at all. `sample` shows the main thread in `_dispatch_semaphore_wait_slow` with all 11 Mojo worker threads parked at the same frame, every stack frame inside the `mojo` binary. The same cases pass in ~5 min via the directory selection, so it is the shape of the unit. Minimise and file upstream. | `conftest.py` `run_with_progress` (done); `marrow/kernels/tests/test_join.mojo` (open) | M |
...
| **B26** | **`PrimitiveArray.__eq__` calls logically-equal arrays unequal, two ways** (`arrays.mojo:749-770`). (a) `if self.bitmap.__bool__() != other.bitmap.__bool__(): return False` — an all-valid array carrying a bitmap compares unequal to one carrying none, despite identical length, `null_count()` and values. Every kernel that intersects validity emits the former; `builders.array([...])` emits the latter, so `AddKernel.apply(a, b) == array([...])` is False for correct output. Found 2026-08-05 while testing Q2.3 — it forced that test to compare elementwise. (b) `self.bitmap.value() == other.bitmap.value()` compares **raw** bitmaps, so two slices at different offsets with identical logical validity compare unequal — Q2.3's own class, in `__eq__`. PyArrow's `Array.equals` treats both as equal. Check the other array types' `__eq__` for the same shape before fixing. | `arrays.mojo:749-770` | S |
