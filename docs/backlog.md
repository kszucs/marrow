...
|---|---|---|
| **A1** | **Replace the positional `Context` with a typed `comptime State` per node**, returned by `prepare` and passed into `vectorwise`. Design validated by spike 2026-08-03 — see `docs/design-expression-evaluation.md`. **Re-scoped 2026-08-05: this is a performance fix, not a tidiness one, and B28 gives it a target.** The fused lane re-resolves every column leaf per SIMD chunk; a hand-written equivalent with the view hoisted runs the same expression **30.5x faster** (`a + 1` over 1M rows: **2.00 ms** today versus **65.7 µs** hoisted, on the same `views.apply` driver). **Design constraint the measurement exposes:** the `State` must hold the *`BufferView`*, not merely the typed array. Hoisting only the schema lookup gets 1.96 -> 1.21 ms, while hoisting everything gets 0.066 ms — so ~1.14 ms of the 1.21 is the `Variant` unwrap plus `.values()` reconstruction, and a `State` holding an owned array would leave most of that in place. The spike validated `State` holding *owning* containers; whether it can hold a borrowed view whose origin is the batch is the **first thing to prove**, before any node is converted. Everything the card previously claimed still holds: the `Context` slot hazard, six methods collapsing to two, ~84 hand-written recursion bodies, B14/B15. **First gate is binary size** — convert three nodes, hold `query_streaming` `__text` at **1,332,456**, stop if it regresses. | L, highest value |
