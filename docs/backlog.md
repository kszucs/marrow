...
| **B27** | **`LengthKernel.apply` runs at ~69 ns/row — roughly 25x slower than the fused lane computing the same thing, and ~85x slower than memory bandwidth allows.** Localised 2026-08-05 with `marrow/expr/tests/bench_fusion_gaps.mojo`, 1M short strings: erased `dispatch` **68.9 ms**, typed `apply` **69.0 ms**, `s.len()` through the expression layer **69.5 ms**, and `s.len() + 0` — strictly more work — **2.72 ms**. The two forms were **verified to produce identical results**, so this is a performance defect, not a correctness one. **Ruled out:** (a) the erasure — typed and erased are within noise of each other, so `variant_dispatch` costs nothing here; (b) a scalar loop — `apply` already vectorises via `Self.core` over loaded offset vectors; (c) constructing the output view per lane — hoisting it out of the closure changed nothing (69.5 vs 68.9). The kernel moves ~8 MB (4 MB of offsets in, 4 MB of int32 out) and takes 69 ms, where bandwidth says under 1 ms. **Prime remaining suspect:** `vectorize[width](n, lane)` is not actually vectorising this body — a degenerate W=1 would give exactly this per-row cost. Check the `fill[W, rank, alignment]` signature against what `vectorize` expects, and whether the `lane[W]` wrapper blocks inlining. **Also measured and separately wrong:** reading a bare string column through the expression layer costs **151 ms** for the same 1M rows — worse than the kernel, and its own defect. | `kernels/string.mojo` `LengthKernel.apply`; `expr/values.mojo` `StringColumn` | M |
| **Q7.1** | **Fusion gaps — measurement attempted first, and it surfaced something else.** The three gaps are real and unchanged: no `Materialized` leaf adapter; `StringPredicate` is a `Breaker` that materialises a full `BoolArray`, so `s1 == s2 and a > b` is two passes; `StringLength` calls `LengthKernel.dispatch` into an `Int32Array`, so `s.len() + a` is two passes. Fusing them means giving `StringValue` a way to expose offsets vectorwise — a change to a trait the size gate watches, so it should be paid for with a measurement rather than a plausible story (Q6.1 is the cautionary case: a fusion that sounded faster measured as identical). `bench_fusion_gaps.mojo` was written to price the extra pass and instead found **B27** — `s.len()` alone is 25x *slower* than `s.len() + 1`. Until that is explained the benchmark cannot price anything, because the baseline is not doing what it appears to. **Do B27 first.** | M each, blocked on B27 |
