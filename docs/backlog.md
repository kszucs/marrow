...
| **B27** | **`s.len()` alone is ~25x slower than `s.len() + 1`.** Measured 2026-08-05 by `marrow/expr/tests/bench_fusion_gaps.mojo`, 1M short strings: `bench_fusion_len_only_1m` **69.0 ms** (sd 0.139) versus `bench_fusion_len_plus_one_1m` **2.73 ms** (sd 0.010). Strictly more work is 25x faster, and the standard deviations rule out noise. Ruled out so far: both go through the same path — `StringLength` conforms to `Breaker`, so `Value.execute` calls `prepare` then reads the slot, and the fused `NumericBinary` above it calls the same `prepare` — and `StringColumn.materialize` returns `batch.columns[i]` as-is rather than rebuilding, so neither should be re-materialising 1M strings. **Not explained.** Candidates: the single-argument `Value.execute` allocating a `Context` per call in a way the fused path avoids; `into_array` copying differently for the two `Datum` shapes; or the harness calibrating the two loops differently. Whatever it is, a 25x inversion on a bare `length()` is worth more than the fusion work it was measured for. | `expr/values.mojo` `StringLength`; `bench_fusion_gaps.mojo` | M |
