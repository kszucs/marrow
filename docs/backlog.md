...
| **B23** | **`pytest marrow/kernels/tests/test_join.mojo` on its own hangs forever, and the harness cannot recover from it.** Reproduced 2026-08-04 at HEAD *and* with the JoinKind change, on two different driver hashes: the run reaches ~10.5 s of CPU and then parks indefinitely (observed at 7 h and 27 min of wall clock against 10.4 s CPU). `sample` shows the main thread in `_dispatch_semaphore_wait_slow` with all 11 Mojo worker threads parked at the same frame, every stack frame inside the `mojo` binary and none in generated code — a deadlock in the toolchain, not a loop in the test. The same cases pass in **4:54** as part of the `marrow/kernels/tests` directory selection, so it is the *shape of the single-file unit* that triggers it, not the code. **Why this matters beyond one file:** `MojoRunner.collect` splits and retries on a compiler *crash*, which produces a signal; a hang produces none, so the harness waits forever and CI would too. Workaround: select the directory, never this file alone. Fix: a per-run timeout in the harness that reports a hang as a failure instead of blocking. | `marrow/kernels/tests/test_join.mojo`; `conftest.py` `MojoRunner.collect` | M |
| **B24** | **`is_fixed_size()` has no production callers and does not mean what its name says.** It answers `is_bool() or is_primitive()` — so `fixed_size_binary` and `fixed_size_list`, the two dtypes whose names promise it, both answer **False**. Only tests call it, and `test_is_fixed_size` (`test_dtypes.mojo:299-305`) does not cover either. Surfaced by the Task 5 review (2026-08-05); pre-existing, and deliberately left alone there because that task was scoped to preserve its answers, not change them. Decide whether it should cover the fixed-size layouts or be deleted. | `dtypes.mojo:1202-1208` | S |
| **B22** | **`Buffer.to_device` -> `to_cpu` loses data, and GPU tests are effectively unrun.** `test_buffers_gpu.mojo` fails 2 of 4 *today*, independent of any recent change: `test_buffer_host_builder` and `test_buffer_to_cpu_round_trip` read back 0 where 99 was written (`test_buffers_gpu.mojo:81`). This is a core transfer defect, not a test bug. It has gone unnoticed because CI passes `--no-gpu` on every job (backlog I-notes) so the five `*_gpu.mojo` files never run there, and a local `pytest --gpu` over a whole directory reports every case in the selection as failed — the file-level rollup — which hides that only two are real. Diagnose with the inner runner summary, not pytest's. | `buffers.mojo` `to_device`/`to_cpu`; `marrow/tests/test_buffers_gpu.mojo:81` | M |
...
| **I4** | **Re-baseline the binary-size numbers.** The live reading is now `query_streaming` __text **1,309,032** (agg_fused 3,748,660; agg 4,122,292; join 3,780,340, all as of 2026-08-05). Q7.4's whole two-commit change cost **0 bytes**; the +8 on `query_streaming` came from B8 narrowing `is_primitive()` by one branch. Against the last recorded baseline (`a8c3e34`: query_streaming 1,309,024; agg_fused 3,748,532; agg 4,122,292; join 3,780,276), `agg` is unchanged but `agg_fused` is +128 and `join` is +64, and neither is attributed to a specific commit — several landed in between besides Q7.3/Q7.4/B8 (a GPU-context fix touching six call sites, the `ExecutionContext` → `ExecContext` rename and its move out of `kernels/`, and two array fixes), and no bisection was done to isolate which one moved these two binaries. Earlier it was 1,309,024 — B12 added 8,260, of which 8,820 is a single `variant_dispatch_raises` instantiation (`DynArray.slice()` over 37 variant members, each arm carrying the null-count call). Any per-arm work in a variant-dispatched method costs ~8 KB; that is the real lever, not the counting code. The recorded 7.6× / 7.8× / 12.8× table predates the interpreter deletion; a local sweep gives 2.83× / 3.12× / 3.01×. Using the written numbers as a gate would invent or hide a regression — the exact failure the docs warn about. Also: `BASELINE.md` is referenced three times and **does not exist**. | `benchmarks/binary_size/README.md:110,154` | S |
...
| **Q4.1** | Missing value types: `Grouping` (`(gids, num_groups)` is still two parameters in eight places), `JoinIndex`, `BuildPartition`. **`JoinKind` is done** — it closed B6 and, on landing, immediately caught a raw `join_kind=0` in `benchmarks/binary_size/query_join.mojo`, which is the class of bug it exists to prevent. Note that file is outside `marrow/`, so `precompile marrow` does not cover it; `pixi run binary_size` is what compiles it. | M |
