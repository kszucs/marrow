...
| **I4** | **Largely RESOLVED by I3.** The baseline now lives in `benchmarks/binary_size/baseline.json` and is regenerated by `check_gate.py --update`, so it cannot drift as prose. Current values (2026-08-05, after `num_buffers`' always-on validation): query_streaming **1,325,672**, query_join 3,816,628, agg_fused 3,786,228, agg 4,159,796. The +1.27% on query_streaming is the deliberate `ArrayData` validation trade — see that commit for the three-way measurement. **Still open:** the recorded 7.6x / 7.8x / 12.8x ratio table in `benchmarks/binary_size/README.md` predates the interpreter deletion; a local sweep gives 2.83x / 3.12x / 3.01x. | `benchmarks/binary_size/README.md` | S |
