...
|---|---|---|
| **A1** | **Replace the positional `Context` with a typed `comptime State` per node.** **Spiked end-to-end 2026-08-05 and the design is validated on the metric that matters** — `marrow/expr/tests/test_a1_spike.mojo`. A column leaf with `comptime State = Int32Array`, a `prepare` that resolves once and a `vectorwise` that only loads, runs `a + 1` over 1M rows in **67.25 µs** against a hand-written hoisted ideal of **67.14 µs** and today's fused lane at **2.0054 ms** — within **0.2%** of the floor, a **29.8x** win. Two things the spike settled: (a) a *parameterised* associated type (`State[o: Origin[mut=False]]`) compiles, but the origin will not unify — `prepare` yields `BufferView[..., origin_of(o.columns[...].buffer)]`, a nested projection of the batch origin, which has no writable name; (b) **it does not matter**, because `State` holding the owned typed array reaches the same number. The costs were the schema lookup and the `Variant` unwrap, both hoisted into `prepare`; `.values()` is pointer arithmetic on a `TrivialRegisterPassable` view and is free per chunk. **So A1 needs no origin gymnastics, no `unsafe_origin_cast` and no parameterised associated types** — materially smaller than the design assumed. Everything previously claimed still holds: the `Context` slot hazard, six methods collapsing to two, ~84 recursion bodies, B14/B15, B28. **First gate is binary size** — convert three nodes, hold `query_streaming` `__text` at **1,332,456**, stop if it regresses. | L, highest value, de-risked |
