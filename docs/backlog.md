...
| **Q7.1** | **Unblocked — B27 is fixed, and it changes the premise.** The three gaps are unchanged, but the cost of a breaker in a fused expression just fell ~2x by removing an atomic from the lane loop, so the remaining gap between "materialise then read" and "truly fused" is smaller than the card assumed. Price it with `bench_fusion_gaps.mojo` before changing `StringValue`'s trait surface — and read the unit on each row, which is what made B27 look like the opposite of what it was. | M each |
