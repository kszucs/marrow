...
|---|---|---|
| **Q0.5** | **Rewritten from the code 2026-08-05.** `Project` and `Aggregate` learn their output dtypes by building a 0-row `RecordBatch.empty(input_schema)` and *executing every expression against it* (`relations.mojo:484` and `:668`). The original card said "`DynValue` should answer its dtype"; the box is `BoxedValue` and it has no `dtype()` at all, which is the actual gap. **But the probe is not simply waste** — both sites carry an argument for it, and it holds: a static rule is incomplete for computed columns (`x + 1`, literals, CASE), and at `:666` the probe is *"the only thing that keeps the declared schema honest: a caller-supplied one asserts the caller's arithmetic, not the plan's"*. So the fix is narrower than "delete the probe": give `BoxedValue` a `dtype()` that the **fused** arm fills from its statically-known `OutType` at box time, and keep the probe for the runtime lane. Claimed ~16 KB and it retires a hand-built `benchmarks/binary_size` exception — now checkable, since the gate is in CI. | M |
